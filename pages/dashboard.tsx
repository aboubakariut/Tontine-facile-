...
export default function Dashboard() {
  ...
  if (authLoading || tontinesLoading) {
    return (
      <>
        <Head>
          <title>Chargement... - Tontine Facile</title>
        </Head>
  ...
  return (
    <>
      <Head>
        <title>Tableau de bord - Tontine Facile</title>
      </Head>
      ...
      <div className="container">
        <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
          <Link href="/create-tontine">
            <button className="btn">+ Créer une tontine</button>
          </Link>
          <button
        ...
        {tontines.length === 0 ? (
          <div className="card" style={{ textAlign: 'center' }}>
            ...
            <Link href="/create-tontine">
              <button className="btn">Créer votre première tontine</button>
            </Link>
          </div>
        ) : (
          <div
          ...
          >
            {tontines.map((tontine: any) => (
              <div key={tontine.id} className="card">
                <h3>{tontine.name}</h3>
                <p>
                  <strong>Montant:</strong> {tontine.amount} FCFA /{' '}
                  {tontine.frequency}
                </p>
                <p>
                  <strong>Membres:</strong> {tontine.members?.length || 0} /{' '}
                  {tontine.total_members}
                </p>
                <p>
